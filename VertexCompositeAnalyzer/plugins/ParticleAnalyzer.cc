...
private:
  ...
  std::vector<std::string> mtdInfo_;
...
ParticleAnalyzer::getEventData(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if (isMC_ && genParticles.isValid())
  {
    ...
    for (size_t i=0; i<genParticles->size(); i++)
    {
      ...
      if (p->isLastCopy() && p->status()==1 && p->charge()!=0 && p->pt()>0.4 && std::abs(p->eta())<3.0)
      {   
        nGenTracks += 1;
      }
...
ParticleAnalyzer::fillEventInfo(const edm::Event& iEvent)
{
  ...
  eventInfo_.add("bestvtxT", vertex_.t());
  eventInfo_.add("bestvtxTerr", vertex_.tError());
...
ParticleAnalyzer::fillTrackInfo(const pat::GenericParticle& cand, const UShort_t& candIdx, const bool& force)
{
  ...
  // MTD information
  for (const auto& s : mtdInfo_) info.add(s, getFloat(cand, s));
  if (!mtdInfo_.empty())
  {
    const float c_cm_ns = 2.99792458e1; //[cm/ns]
    const auto tMTDErr = getFloat(cand, "tMTDErr");
    const auto dt = getFloat(cand, "tMTD") - vertex_.t();
    const auto beta = tMTDErr>=0. ? (getFloat(cand, "pathLength")/dt)*(1./c_cm_ns) : -99.;
    const auto gammaSq = tMTDErr>=0. ? 1./(1.0-std::pow(beta, 2.0)) : -99.;
    const auto massSq = tMTDErr>=0. ? std::pow(track.p(), 2.0)/(gammaSq - 1.) : -99.;
    info.add("beta", beta);
    info.add("gammaSq", gammaSq);
    info.add("massSq", massSq);
  }
...
ParticleAnalyzer::loadConfiguration(const edm::ParameterSet& config, const edm::Run& iRun)
{
  ...
  if (mtdInfo_.empty())
  {
    mtdInfo_ = (config.existsAs<std::vector<std::string> >("mtdValueNames") ? config.getParameter<std::vector<std::string> >("mtdValueNames") : std::vector<std::string>());
  }
