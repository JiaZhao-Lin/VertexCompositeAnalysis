...
class ParticleDaughter {
 ...
 private:
  ...
  void setMTD(pat::GenericParticle& c, const std::map<std::string, edm::Handle<edm::ValueMap<float> > >& m);
  ...
  std::map<std::string, edm::EDGetTokenT<edm::ValueMap<float> > > token_mtd_map_;
};
