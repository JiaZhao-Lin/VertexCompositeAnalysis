...
void ParticleDaughter::fillInfo(const edm::ParameterSet& pSet, const edm::ParameterSet& config, edm::ConsumesCollector& iC)
{
  ...
  if (config.existsAs<std::vector<std::string> >("mtdValueLabels")) {
    const auto& labels = config.getParameter<std::vector<std::string > >("mtdValueLabels");
    const auto& names = config.getParameter<std::vector<std::string > >("mtdValueNames");
    for (size_t i=0; i<labels.size(); i++) {
      token_mtd_map_.emplace(names[i], iC.consumes<edm::ValueMap<float> >(edm::InputTag(labels[i])));
    }
  }
};
...
void ParticleDaughter::addParticles(const edm::Event& event, const edm::EDGetTokenT<std::vector<T> >& token, const reco::Vertex& vertex, const bool embedInfo)
{
  ...
  std::map<std::string, edm::Handle<edm::ValueMap<float> > > mtdMap;
  for (const auto& s : token_mtd_map_) {
    if (!token_dedx_.isUninitialized()) event.getByToken(s.second, mtdMap[s.first]);
  }
  // set selections
...
void ParticleDaughter::setMTD(pat::GenericParticle& c, const std::map<std::string, edm::Handle<edm::ValueMap<float> > >& mtdMap)
{
  const auto& track = (c.hasUserData("trackRef") ? *c.userData<reco::TrackRef>("trackRef") : c.track());
  if (track.isNull()) return;
  for (const auto& s : mtdMap) {
    if (s.second.isValid() && s.second->contains(track.id())) {
      const auto& value = (*s.second)[track];
      c.addUserFloat(s.first, value);
    }
  }
};
